 ...
 * MultiVideos
 * 
 * Добавление видеогалереи к странице
 *
 * @category 	plugin
 * @version 	1.0
 * @license 	http://www.gnu.org/copyleft/gpl.html GNU Public License (GPL)
 * @author      Pathologic (maxx@np.by)
 * @internal	@properties &tvIds=TV Ids;text;&templ=Template;text;&role=Role;text; &w=Preview: width;text; &h=Preview: height;text;120 &thumbUrl=Thumbs folder;text;assets/images/video/ &forceDownload=Force download;list;Yes,No;No
...
foreach ($tvIds as $tvid) {
	...
	foreach ($videoArr as $k=>&$v) {
		if (empty($v[1])) {
      $preview = $thumb->process($v[0],true);
